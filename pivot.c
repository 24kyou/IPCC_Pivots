...
double SumDistance(const int k, const int n, const int dim, double* coord, int* pivots){
    ...
    double distance = 0;
    #pragma omp parallel for num_threads(n/100) reduction(+ : distance)
    for(i=0; i<n; i++){
        int ki;
        for(ki=0; ki<k; ki++){
            double distance = 0;
    ...
    #pragma omp parallel for num_threads(n/100) reduction(+ : chebyshevSum)
    for(i=0; i<n; i++){
        int j;
        for(j=i+1; j<n; j++){
            double chebyshev = 0;
    ...
    return 2*chebyshevSum;
}
int Partition(double* data, int start, int end)   //划分数据
{
    double temp = data[start];   //以第一个元素为基准
    while (start < end) {
...
void quickSort(double* data, int start, int end)  //并行快排
{
    if (start < end) {
        double pos = Partition(data, start, end);
            ...
            #pragma omp parallel sections    //设置并行区域
        {
            #pragma omp section
            {
                quickSort(data, start, pos - 1);//该区域对前部分数据进行排序
            }
            
            #pragma omp section
            {
                quickSort(data, pos + 1, end);//该区域对后部分数据进行排序
            }
        }
        }
...
void Combination(int ki, const int k, const int n, const int dim, const int M, double* coord, int* pivots,
    ...
    if(ki==k-1){
        ...
        for(i=pivots[ki-1]+1; i<n; i++){
            ...
            int kj;
            //#pragma omp parallel for num_threads(n/100)
            for(kj=0; kj<k; kj++){
        ...
        quickSort(maxDistanceSum, 0, M - 1);
        quickSort(minDistanceSum, 0, M - 1);
        return;
    }
...
int main(int argc, char* argv[]){
    ...
    printf("%lf\n", minDistanceSum[0]);
    //printf("t2-t1:%ld   t4-t3:%ld",t5,t6);
    // for(i=0; i<M; i++){
